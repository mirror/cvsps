...
class CVSRepository:
    ...
    def convert(self, module, gitdir, more_cvsps_opts=''):
        ...
        do_or_die('cvsps {3} {5} --root ":local:{0}" --fast-export {1} | {4} | (cd {2} >/dev/null; git fast-import --quiet --done && git checkout)'.format(self.directory, module, gitdir, vopt, "tee %s.log" % module if vopt else "cat", more_cvsps_opts))
    def cleanup(self):
